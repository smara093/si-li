...
test('items are displayed in the correct order, group by active status, order by active status asc, date desc when list is updated', () => {
  const items = [
    {
      text: 'item0',
      id: 0,
...
test('items are displayed in the correct order, when list is selected', () => {
  const items = [
    {
      text: 'item0',
      id: 0,
  ...
  const action = { type: types.LISTS_SELECTED_LIST, data: { items } };
  const result = simpleListReducer(undefined, action);
...
test('empty list is returned when list is updated to empty list', () => {
  const items = null;
...
test('can update the new item text without affecting the existing items', () => {
  ...
  const action = { type: types.LIST_UPDATED_TEXT, data: 'New Value' };
